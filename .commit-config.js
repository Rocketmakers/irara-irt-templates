...
const types = [
  ...
  },
  {
    name: "release",
    description:
      "Changes made during the release process (e.g. changelog commit)",
    changelogHeading: "",
    allowBreakingChanges: true,
  },
  {
...
const longestTypeName = types.reduce(
  (longest, type) => Math.max(type.name.length, longest),
  0,
);
...
function buildCommitizenTypeConfig(type, nameIndentSize) {
  ...
  const spaces = Array.from(
    { length: nameIndentSize - name.length },
    () => " ",
  ).join("");
